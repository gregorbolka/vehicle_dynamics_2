...
%torque_vec
%   Generating torque vectoring signals for a double lane change
...
%   Torque are only apply to rear wheels.
...
dT = max(-200, min((delta_now - delta_old)*K, 200)); % -200Nm <= dT <= 200 Nm
dTrl = -dT;
dTrr = dT;
...
if abs(angles(3)) > 0.06 || sqrt(Fxrl^2+Fyrl^2) > vehicle_param.mu*Fzrl*0.75
    dTrl = -min(200, max(180, vehicle_param.mu*Fzrl*0.8));
end
if abs(angles(4)) > 0.06 || sqrt(Fxrr^2+Fyrr^2) > vehicle_param.mu*Fzrr*0.75
    dTrr = -min(200, max(180, vehicle_param.mu*Fzrl*0.8));
end
